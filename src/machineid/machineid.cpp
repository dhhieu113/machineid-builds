...
void getMacHash(unsigned short &mac1, unsigned short &mac2) {
    ...
    for (struct ifaddrs* ifap = ifaphead; ifap; ifap = ifap->ifa_next) {
        ...
        if (sdl && (sdl->sdl_family == AF_LINK) && (sdl->sdl_type == IFT_ETHER)) {
            if (!foundMac1) {
                foundMac1 = true;
                mac1 = hashMacAddress((unsigned char*)(LLADDR(sdl)));
            } else {
    ...
    for (ifr = conf.ifc_req; (char *)ifr < (char *)conf.ifc_req + conf.ifc_len; ifr++) {
        if (memcmp(ifr->ifr_addr.sa_data, (ifr + 1)->ifr_addr.sa_data, 6) == 0)
...
#else
#warning "getMacHash not implemented for this platform"
#endif
